...
|---|---|---|
| use/has terrain semantics | **Yes** | VLMs/CLIP used for terrain-type understanding from aerial + front camera imagery. |
| use/has physical properties | **Yes** | Physically grounded via proprioception: deformability (legs) and slipperiness (wheels) inform traversability. |
| use/has geometric properties/elevation | **Yes** | LiDAR + aerial/top-down imagery capture 3D structure/obstacles; used in local/global reasoning. |
| use/has proprioception | **Yes** | Joint forces (legs) and wheel-vs-LiDAR odometry (wheels) produce τ_sinkage / τ_slip. |
| has planning or control loop | **Yes** | Global VLM-based waypoint selection + local DWA with frontier cost; continuous re-estimation and replanning. |
| use/has classical mapping | **Partial** | Uses frontiers and overlays rather than a persistent occupancy/cost map; emphasis on VLM reasoning + exemplars. |
| use/has learning-based mapping | **Yes** | In-context learning updates terrain traversability from proprioceptive exemplars (online adaptation). |
| needs lidar | **Yes** | Required for slippage estimation (compare against wheel odom) and obstacle sensing. |
| works on quadrupeds | **Yes** | Validated on Ghost Vision 60 (legged) and Clearpath Husky (wheeled). |
