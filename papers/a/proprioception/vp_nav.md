...
### ✅ VP-Nav System Properties Checklist

| Category | Description | Yes/No | Evidence |
|-----------|--------------|--------|-----------|
| **Uses / has terrain semantics** | Does not include semantic labels (e.g., stairs, grass) — only geometry and physical cues. | **No** | Lacks semantic mapping; 2D occupancy only |
| **Uses / has physical properties** | Incorporates proprioceptive sensing of slipperiness, softness, and collisions. | **Yes** | Safety Advisor uses proprioception to detect slipperiness & payload effects |
| **Uses / has geometric properties / elevation** | Builds 2D occupancy and cost maps; does not use full 3D elevation. | **Partial** | 2D top-down occupancy from depth, not elevation map |
| **Uses / has proprioception** | Core to the method; used for collision and fall detection. | **Yes** | “Coupling vision and proprioception” is central to design |
| **Has planning or control loop** | Has asynchronous planner + locomotion control loop (10 Hz planning, 100 Hz control). | **Yes** | Planner & policy run in loop onboard |
| **Uses / has classical mapping** | Builds 2D occupancy & FMM-based cost map (classical). | **Yes** | FMM geodesic + SDF cost map |
| **Uses / has learning-based mapping** | No; mapping is geometric, not learned. | **No** | Mapping via librealsense and FMM |
| **Needs LiDAR** | Uses depth cameras (RealSense D435), not LiDAR. | **No** | Explicitly depth camera-based |
| **Works on quadrupeds** | Designed and tested on Unitree A1 quadruped. | **Yes** | Implemented and deployed on Unitree A1 |