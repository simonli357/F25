...
|---|---|---|
| use/has terrain semantics | **Yes** | Terrain classifier (BEV patches) assigns traversability costs. |
| use/has physical properties | **Yes (implicit)** | Physical effects captured via proprioception/value function used to adjust costs online. |
| use/has geometric properties/elevation | **Partial** | Uses depth→point cloud→SDF for obstacles; no explicit elevation map layer. |
| use/has proprioception | **Yes** | RL critic (motion evaluation) used as proprioception advisor to detect slips/collisions and inject costs. |
| has planning or control loop | **Yes** | Closed-loop: planner over multi-layer cost map + RL locomotion controller feedback. |
| use/has classical mapping | **Yes** | Maintains classical cost maps (terrain, obstacles, proprioception, goal) with dynamic weights. |
| use/has learning-based mapping | **Yes** | CNN-based terrain estimator + learned RL critic; latent-NN online corrections. |
| needs lidar | **No** | RGB-D (RealSense) only; LiDAR mentioned only as a possible future odometry aid. |
| works on quadrupeds | **Yes** | Implemented/tested on Unitree Go1/Go2. |
