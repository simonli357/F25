...
## 16) EVORA Capability Summary

| Category | Present (✓) | Description |
|-----------|--------------|-------------|
| Uses / has terrain semantics | ✓ | Employs semantic octomap from LiDAR + image segmentation (PointRend on RUGD dataset) to incorporate terrain class information. |
| Uses / has physical properties | ✓ | Models physical traction — the ratio between achieved and commanded velocities — as the key traversability property. |
| Uses / has geometric properties / elevation | ✓ | Incorporates elevation patches and geometric context as map inputs alongside semantics. |
| Uses / has proprioception | ✓ | Derives traction labels from proprioceptive feedback (achieved vs commanded velocities). |
| Has planning or control loop | ✓ | Implements risk-aware Model Predictive Path Integral (MPPI) control using CVaR-based planning. |
| Uses / has classical mapping | ✓ | Builds semantic elevation maps and octomaps for spatial structure and semantic fusion. |
| Uses / has learning-based mapping | ✓ | Leverages evidential deep learning and normalizing flows for uncertainty-aware traction estimation and OOD detection. |
| Needs LiDAR | ✓ | Requires LiDAR for map reconstruction and semantic fusion. |
| Works on quadrupeds | ✓ | Validated on both wheeled and quadruped robots (e.g., Boston Dynamics Spot). |
