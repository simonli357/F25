...
## WVN Capability Checklist

| **Property** | **WVN** | **Notes / Evidence** |
|---------------|----------|----------------------|
| **Uses / has terrain semantics** | ✅ Yes | WVN uses DINO-ViT and STEGO features that encode terrain semantics (e.g., grass vs. dirt) for traversability learning. |
| **Uses / has physical properties** | ⚠️ Limited / Indirect | Physical interaction (traction via proprioceptive feedback) generates self-supervised labels, but no explicit modeling of material stiffness or friction. |
| **Uses / has geometric properties / elevation** | ✅ Optional | Elevation maps (2.5D) from LiDAR/depth sensors are used for fusion and local planning, not for primary learning. |
| **Uses / has proprioception** | ✅ Yes | Proprioceptive data (velocity tracking error) provides the online self-supervised traversability signal. |
| **Has planning or control loop** | ✅ Yes | Integrated with local terrain mapping and reactive planner for closed-loop navigation on ANYmal robots. |
| **Uses / has classical mapping** | ✅ Yes | Raycasting into a 2.5D elevation map for geometric fusion and local planning. |
| **Uses / has learning-based mapping** | ✅ Yes | Traversability learned online via neural MLP and pre-trained feature embeddings (learning-based visual map). |
| **Needs LiDAR** | ❌ No (Optional) | LiDAR/depth used only for optional terrain mapping; core WVN learning runs on RGB + odometry/proprioception. |
| **Works on quadrupeds** | ✅ Yes | Demonstrated on ANYmal C and D legged robots for forest, park, and mixed indoor-outdoor navigation. |