...
### ✅ Wild Visual Navigation (WVN) Feature Summary

| Category | Yes/No | Explanation |
|-----------|---------|-------------|
| **Uses / Has Terrain Semantics** | ✅ Yes | Uses DINO-ViT semantic visual features and superpixel segmentation to represent terrain appearance. |
| **Uses / Has Physical Properties** | ❌ No | Does not explicitly model physical properties like friction or compliance; relies on vision + velocity error proxy. |
| **Uses / Has Geometric Properties / Elevation** | ⚠️ Partial | Uses 2.5D elevation maps only for projection/raycasting and SDF creation (geometry assists mapping, not learning). |
| **Uses / Has Proprioception** | ✅ Yes | Uses velocity tracking error from proprioceptive feedback as self-supervision for traversability scoring. |
| **Has Planning or Control Loop** | ✅ Yes | Closed-loop integration: SDF-based local planner + learning-based locomotion control. |
| **Uses / Has Classical Mapping** | ✅ Yes | Fuses traversability predictions into 2.5D elevation map (classical mapping component). |
| **Uses / Has Learning-Based Mapping** | ✅ Yes | Traversability layer learned online using self-supervised MLP + ViT features. |
| **Needs LiDAR** | ⚠️ Optional / Partial | LiDAR used only for terrain map generation (not for traversability learning). |
| **Works on Quadrupeds** | ✅ Yes | Implemented and tested on ANYmal C quadruped; generalizable to other legged or ground robots. |
