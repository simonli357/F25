...
| Feature / Property | TerraPN Description | Presence |
|---------------------|--------------------|-----------|
| **Uses / has terrain semantics** | Uses *weak segmentation* to identify regions of differing pixel intensities (not high-level terrain semantics) — no semantic labels or terrain classes. | ⚠️ *Partial* |
| **Uses / has physical properties** | Explicitly learns surface *physical properties* (traction, bumpiness, deformability) from IMU and odometry errors. | ✅ *Yes* |
| **Uses / has geometric properties / elevation** | Does **not** use elevation or geometric slope; authors note as a limitation. | ❌ *No* |
| **Uses / has proprioception** | Yes — uses IMU and wheel odometry as proprioceptive inputs for self-supervision. | ✅ *Yes* |
| **Has planning or control loop** | Yes — modifies Dynamic Window Approach (DWA) to include surface cost and adaptive acceleration limits. | ✅ *Yes* |
| **Uses / has classical mapping** | Weak segmentation and patch-based cost map construction — not classical mapping (no SLAM or elevation map). | ⚠️ *Partial* |
| **Uses / has learning-based mapping** | Yes — self-supervised neural network produces learned surface cost maps. | ✅ *Yes* |
| **Needs LiDAR** | Used only for odometry and obstacle detection (not required for inference or cost prediction). | ⚠️ *Optional* |
| **Works on quadrupeds** | Evaluated on wheeled Husky robot; no quadruped tests or dynamics considered. | ❌ *No* |
