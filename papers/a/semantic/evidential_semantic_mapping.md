...
## 16) Category Checklist

| Category | Present / Used | Notes |
|-----------|----------------|-------|
| **Uses/has terrain semantics** | ✅ Yes | Performs 3D **semantic mapping** of terrain elements (e.g., dirt, grass, vegetation, unpaved roads) from RGB-LiDAR fusion. |
| **Uses/has physical properties** | ❌ No | Focuses on semantics and uncertainty; no modeling of terrain friction, deformability, or physical traversability. |
| **Uses/has geometric properties / elevation** | ✅ Yes | Incorporates **geometric mapping** via LiDAR-based 3D projection and Bayesian Kernel Inference for continuous voxel geometry. |
| **Uses/has proprioception** | ❌ No | No proprioceptive sensing; purely **exteroceptive** (RGB + LiDAR). |
| **Has planning or control loop** | ❌ No | Performs mapping only; no integration with control or motion planning. |
| **Uses/has classical mapping** | ✅ Yes | Uses **Bayesian Kernel Inference (BKI)** — a classical probabilistic mapping approach. |
| **Uses/has learning-based mapping** | ✅ Yes | Incorporates **Evidential Deep Learning (EDL)** for uncertainty-aware semantic segmentation. |
| **Needs LiDAR** | ✅ Yes | LiDAR is required for spatial projection and map fusion. |
| **Works on quadrupeds** | ⚠️ Likely, but not evaluated | Designed for ground/off-road robotics; adaptable to quadrupeds if pose estimation is available. |
