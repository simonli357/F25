...
### 15. Capability Checklist

| Category | Present / Used in VERN | Evidence / Explanation |
|-----------|------------------------|------------------------|
| **Uses / has terrain semantics** | ✅ Yes | Differentiates vegetation classes (sparse grass, dense grass, bush, tree) by visual appearance and pliability to assign semantic costs. |
| **Uses / has physical properties** | ✅ Yes | Explicitly models **pliability (flexibility)** and vegetation **height/density**, which affect resistance to motion. |
| **Uses / has geometric properties / elevation** | ✅ Yes | Builds **multi-height LiDAR cost maps (low/mid/high)** to infer obstacle height and geometry; combines via element-wise sum (Ccrit). |
| **Uses / has proprioception** | ⚠️ Partially (planned future work) | Spot’s proprioception is not used in current pipeline; authors note adding it for occluded terrain as **future work**. |
| **Has planning or control loop** | ✅ Yes | Adapts **Dynamic Window Approach (DWA)** local planner; integrates cost map, cautious behaviors, and **holonomic recovery control loop**. |
| **Uses / has classical mapping** | ✅ Yes | Builds **occupancy grids / cost maps** from 2D and 3D LiDAR scans, projected via **homography**. |
| **Uses / has learning-based mapping** | ✅ Yes | Employs a **few-shot siamese learning classifier** for vegetation classification fused into mapping process. |
| **Needs LiDAR** | ✅ Yes | Core to multi-view cost maps and height estimation (VLP-16 3D LiDAR required). |
| **Works on quadrupeds** | ✅ Yes | Implemented and validated on **Boston Dynamics Spot** quadruped robot. |

---

✅ = Yes ⚠️ = Partially / Future work ❌ = No
