...
## 15. TerrainNet Capability Checklist

| Feature / Capability | TerrainNet Status | Notes |
|-----------------------|------------------|-------|
| **Uses / Has Terrain Semantics** | ✅ Yes | Predicts semantic BEV maps (dirt, grass, bush, canopy, tree, rock, etc.) via multi-head decoder. |
| **Uses / Has Physical Properties** | ⚪ Partial | Infers physical traits indirectly (e.g., hardness, friction, traversability) via semantics and height; no explicit physical modeling. |
| **Uses / Has Geometric Properties / Elevation** | ✅ Yes | Predicts multi-layer elevation maps: min/max ground and ceiling heights (h_min, h_max, h_ceiling). |
| **Uses / Has Proprioception** | ⚪ Partial | Not proprioceptive per se—uses vehicle-centered BEV and orientation-stable aggregation, but no internal sensing of joint states or IMU fusion. |
| **Has Planning or Control Loop** | ✅ Yes | Integrated with an MPPI planner; produces costmaps for real-time off-road planning. |
| **Uses / Has Classical Mapping** | ⚪ Partial | Generates local BEV maps (not global SLAM); ground-truth maps built from LiDAR offline, but runtime mapping is feed-forward and learning-based. |
| **Uses / Has Learning-Based Mapping** | ✅ Yes | Fully end-to-end neural network predicts semantics + geometry from RGB/stereo inputs. |
| **Needs LiDAR** | ❌ No (at runtime) | LiDAR used only for training supervision (depth/semantic labels); inference is camera-only. |
| **Works on Quadrupeds** | ⚪ Potentially | Designed for wheeled off-road vehicles; adaptable to quadrupeds, but untested (depends on costmap-planner integration). |
