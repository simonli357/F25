...
### ✅ Semantic Traversability – Property Checklist

| Category | Description | Present? | Evidence / Notes |
|-----------|--------------|-----------|------------------|
| **Uses / has terrain semantics** | Semantic traversability estimator trained on egocentric RGB video using SAM + Mask2Former semantic cues | ✅ | Semantic preferences (crosswalks good, roads less preferred); SegFormer fine-tuned for per-pixel traversability. |
| **Uses / has physical properties** | Explicit modeling of friction, roughness, or compliance | ❌ | Purely vision-based semantics; no explicit physical property layer. |
| **Uses / has geometric properties / elevation** | Incorporates geometric traversability via depth-based 2.5D grid | ✅ | 2.5D grid with semantic + geometric layers; elevation from RGB-D depth. |
| **Uses / has proprioception** | Relies on robot internal sensing (IMU, force, torque) | ⚠️ Partial | Uses IMU in ORB-SLAM3 for scale during annotation; no robot force/torque in the loop. |
| **Has planning or control loop** | Integrated into path planning or control | ✅ | RRT* planner uses fused geometric + semantic cost layers. |
| **Uses / has classical mapping** | Builds metric/elevation map from sensors | ✅ | Builds 2.5D grid maps (10 m × 10 m, 0.025 m resolution). |
| **Uses / has learning-based mapping** | Learns semantic traversability directly from vision | ✅ | SegFormer-based model outputs per-pixel traversability from RGB. |
| **Needs LiDAR** | Requires LiDAR for operation | ❌ | Uses RGB or RGB-D cameras only; LiDAR not required. |
| **Works on quadrupeds** | Tested on legged robots | ✅ | Deployed on Raibo 2 quadruped for urban navigation. |
