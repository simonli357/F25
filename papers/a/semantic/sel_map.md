...
## Technical Characterization Checklist

| Category | Description / Evidence | ✓ / ✗ |
|-----------|------------------------|-------|
| **Uses / has terrain semantics** | The method applies semantic segmentation to label each mesh face with terrain classes (e.g., grass, rock, ice). | ✓ |
| **Uses / has physical properties** | It explicitly estimates terrain physical properties (coefficient of friction) using a Bayesian framework and class-conditioned Gaussian models. | ✓ |
| **Uses / has geometric properties / elevation** | Constructs a recursive 2.5D triangular elevation mesh with per-vertex height and variance estimates (Kalman filter). | ✓ |
| **Uses / has proprioception** | Uses camera pose and pose covariance (from robot’s odometry or SLAM) but not internal joint state feedback; limited proprioceptive use. | Partial (✓/✗) |
| **Has planning or control loop** | The paper provides mapping only; it does not include a planning or control component (no MPC, gait adaptation, or control loop). | ✗ |
| **Uses / has classical mapping** | Yes — geometric elevation mapping via recursive Kalman filtering is a classical probabilistic mapping technique. | ✓ |
| **Uses / has learning-based mapping** | Semantic segmentation relies on CNNs trained on ADE20K, but mapping recursion itself is analytical (Bayesian); thus, partially learning-based. | Partial (✓/✗) |
| **Needs LiDAR** | No; it uses only a single RGB-D camera, though LiDAR extension is discussed. | ✗ |
| **Works on quadrupeds** | Deployed and tested on Boston Dynamics Spot (a quadruped) and Agility Digit (biped), validated in real-world scenes. | ✓ |
