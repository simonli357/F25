...
| Category | Presence / Usage | Evidence |
|-----------|------------------|-----------|
| **Uses / has terrain semantics** | ✅ Yes | Integrates semantic segmentation (YOLACT-based) into elevation mapping to distinguish ground, stairs, pedestrians, obstacles |
| **Uses / has physical properties** | ❌ No | Only geometric height and semantic categories; no mention of friction, roughness, or material properties |
| **Uses / has geometric properties / elevation** | ✅ Yes | Constructs 2.5D elevation map from point cloud depth using Kalman filter–based height fusion |
| **Uses / has proprioception** | ❌ No | No robot state feedback, kinematics, or IMU-based localization used; only RGB-D input |
| **Has planning or control loop** | ❌ No | Ends at mapping; no integration with control or navigation modules |
| **Uses / has classical mapping** | ✅ Yes | Employs grid-based 2.5D elevation maps similar to Fankhauser’s probabilistic terrain mapping |
| **Uses / has learning-based mapping** | ✅ Yes | Incorporates deep learning (improved YOLACT with hierarchical residual conv blocks) for semantic segmentation |
| **Needs LiDAR** | ❌ No | Uses RGB-D (Intel RealSense D455) for point cloud generation |
| **Works on quadrupeds** | ⚠️ Not directly | Designed for general mobile robots; cites quadruped terrain optimization work but does not integrate locomotion control |
