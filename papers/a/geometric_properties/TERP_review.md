...
| Category | Description | TERP (Yes/No) | Notes |
|-----------|--------------|---------------|-------|
| **Uses/has terrain semantics** | Semantic understanding (e.g., classifying grass, rock, road) | **No** | Uses only geometric/elevation cues; no semantic segmentation |
| **Uses/has physical properties** | Accounts for physical stability or dynamics (e.g., roll, pitch, torque) | **Yes** | Cost-map encodes stability, and unreachable areas are marked ∞ based on torque limits |
| **Uses/has geometric properties / elevation** | Uses geometric features like slope, elevation, gradient | **Yes** | Core input is normalized elevation maps from LiDAR |
| **Uses/has proprioception** | Robot’s internal state (pose, orientation, roll, pitch) used in planning | **Yes** | Inputs include pose, roll, pitch, heading, and goal direction |
| **Has planning or control loop** | Planning/control feedback loop beyond perception | **Yes** | Two-stage loop: waypoint planning + DWA-RL control |
| **Uses/has classical mapping** | Classical methods (e.g., OctoMap, Grid-Map) | **Yes** | Uses OctoMap and Grid-Map for elevation construction |
| **Uses/has learning-based mapping** | Map built via learning (semantic or implicit features) | **Partial** | Learns *attention weights* on elevation maps but not the map itself |
| **Needs LiDAR** | Requires LiDAR input for operation | **Yes** | Uses Velodyne VLP-16 3D LiDAR for elevation data |
| **Works on quadrupeds** | Designed/tested for quadruped robots | **No** | Implemented/tested on a wheeled Husky robot |
