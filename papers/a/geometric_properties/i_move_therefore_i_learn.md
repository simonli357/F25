...
## Capability and System Property Checklist

| Capability / Property                          | Present? | Notes |
|---|---|---|
| use/has terrain semantics | No | Operates on RGB + elevation similarity without explicit semantic classes. |
| use/has physical properties | Partial | Mentions potential future inclusion of friction/stiffness; current method uses appearance + elevation only. |
| use/has geometric properties/elevation | Yes | Builds multi-layer grid maps with an elevation layer from LiDAR fused with RGB. |
| use/has proprioception | No | Relies on exteroceptive sensors (camera & LiDAR); proprioceptive cues are discussed as future work. |
| has planning or control loop | Yes | Produces a traversability layer consumed by Nav2 for planning/control. |
| use/has classical mapping | Yes | Uses classical multi-layer grid maps (grid_map / OccupancyGrid). |
| use/has learning-based mapping | Yes | Patch encoding via VAE and experience summarization via BIRCH clustering. |
| needs lidar | Yes | LiDAR used to provide elevation for RGB–LiDAR fusion. |
| works on quadrupeds | Yes | Validated on Unitree Go2 (quadruped) as well as a wheeled platform. |
