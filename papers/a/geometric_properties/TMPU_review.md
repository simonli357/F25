...
## 15. Framework Feature Checklist

| Category | Description | TMPU Status |
|-----------|--------------|-------------|
| **Uses / has terrain semantics** | Terrain is analyzed geometrically (slope, roughness, height) without semantic labeling (e.g., grass, road). | ❌ |
| **Uses / has physical properties** | Incorporates physical terrain features like **slope** and **roughness**, linked to robot tire contact safety. | ✅ |
| **Uses / has geometric properties / elevation** | Explicitly models **plane geometry** and **height differences**; builds 2D maps from 3D OctoMap. | ✅ |
| **Uses / has proprioception** | Does not use onboard proprioceptive feedback (forces, IMU, joint states); relies solely on LiDAR mapping. | ❌ |
| **Has planning or control loop** | Yes — includes **A\***, **H-lattice**, and a **local controller (ROS move_base)** for closed-loop navigation. | ✅ |
| **Uses / has classical mapping** | Employs **OctoMap (3D occupancy)** and converts it into a **2D occupancy grid**. | ✅ |
| **Uses / has learning-based mapping** | No learning or data-driven components; purely analytical mapping. | ❌ |
| **Needs LiDAR** | Yes — depends on **LOAM** (LiDAR Odometry and Mapping) to generate OctoMap. | ✅ |
| **Works on quadrupeds** | Evaluated only on a **ground wheeled robot (Scout 2.0)**; not designed for legged robots. | ❌ |