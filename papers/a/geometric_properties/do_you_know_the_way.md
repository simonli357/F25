...
| Category | Present? | Explanation |
|-----------|-----------|-------------|
| **Uses/has terrain semantics** | ⚪ **Partial** | Uses visual traversability cues but not explicit semantic terrain classes. |
| **Uses/has physical properties** | ❌ **No** | Does not model friction, roughness, or traction; purely vision-based. |
| **Uses/has geometric properties / elevation** | ✅ **Yes** | Fuses per-pixel traversability into a multi-modal elevation map and BEV projection. |
| **Uses/has proprioception** | ❌ **No** | Vision-only perception; no proprioceptive data used. |
| **Has planning or control loop** | ✅ **Yes** | Integrated with Model Predictive Path Integral (MPPI) control for navigation. |
| **Uses/has classical mapping** | ✅ **Yes** | Employs an elevation-based mapping layer for fusing learned traversability. |
| **Uses/has learning-based mapping** | ✅ **Yes** | Traversability layer learned online via DINOv2 features + MLP head retraining. |
| **Needs LiDAR** | ⚪ **Optional / No (Vision-only)** | Works without LiDAR; baselines used LiDAR for comparison only. |
| **Works on quadrupeds** | ⚪ **Not demonstrated** | Tested on Clearpath Jackal (wheeled); adaptable to quadrupeds but untested. |
